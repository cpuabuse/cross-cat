...
function number(text: string, countBlank: boolean) {
	...
	if (countBlank) {
		maxNumberOfDigits = lines.length.toString().length;
	} else {
		maxNumberOfDigits = lines
	...
	// As we are splitting the lines variable from a string it will always contain a value
